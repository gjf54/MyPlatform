...
<div class="products">
    ...
    <script>
        add_url = "{{ route('add_amount', ['id' => $element->id]) }}"
        rem_url = "{{ route('rem_amount', ['id' => $element->id]) }}"
        rem_el_url = "{{ route('rem_element', ['id' => $element->id]) }}"
    </script>
    ...
    <div class="product d-flex justify-content-between align-items-center" id="<?= 'element-' . $element->id?>">
        ...
        <div class="product_control_buttons d-flex flex-row justify-content-center align-items-center">
            <div class="plus_button" onclick="add_amount('<?= route('add_amount', ['id' => $element->id]) ?>')"></div>
            <span id="<?= 'amount-' . $element->id ?>" class="d-flex justify-content-center align-items-center">{{ $element->amount }}</span>
            <div class="minus_button" onclick="rem_amount('<?= route('rem_amount', ['id' => $element->id]) ?>')" ></div>
            <div class="remove_button" onclick="rem_element('<?= route('rem_element', ['id' => $element->id]) ?>')"></div>
        </div>
    </div>
    @endforeach
</div>
